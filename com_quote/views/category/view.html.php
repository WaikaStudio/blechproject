...
/**
 * HTML View class for the Quote component
 *
 * @package     Quote
 * @subpackage  Views
 */
class QuoteViewCategory extends JViewCategory
{
	/**
	...
	protected function prepareDocument()
	{
		...
		if ($menu && ($menu->query['option'] != 'com_quote' || $menu->query['view'] == 'quote' || $id != $this->category->id))
		{
			$path = array(array('title' => $this->category->title, 'link' => ''));
			$category = $this->category->getParent();

			while (($menu->query['option'] != 'com_quote' || $menu->query['view'] == 'quote' || $id != $category->id) && $category->id > 1)
			{
				$path[] = array('title' => $category->title, 'link' => QuoteHelperRoute::getCategoryRoute($category->id));
